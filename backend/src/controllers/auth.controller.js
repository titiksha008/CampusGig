...
const cookieOpts = {
  ...
  // path: "/",                   // optional; uncomment if you need it
};
...
export const updateProfile = async (req, res) => {
  try {
    ...
    res.json({ ...updatedUser.toObject(), skills: updatedUser.skills || [] });
  } catch (e) {
...
export const logout = (req, res) => {
  // Must mirror the same cookie attributes used when setting it
  res.clearCookie("token", {
    httpOnly: true,
    secure: isProd,
    sameSite: isProd ? "none" : "lax",
    // path: "/",                // include if you set it when creating the cookie
  });
